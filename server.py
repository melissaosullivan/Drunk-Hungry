...
def foursquare():
    ...
    for venue in blob['groups'][0]['items']:
        try:
            result.append({"name" : venue['venue']['name'], "photo" : venue['venue']['photos']['groups'][0]['items'][0]['prefix'] + '400x400' + venue['venue']['photos']['groups'][0]['items'][0]['suffix'], "lat" : venue['venue']['location']['lat'], "lng" : venue['venue']['location']['lng']})
        except Exception, e:
            pass
